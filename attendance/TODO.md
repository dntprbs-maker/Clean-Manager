...
- [ ] **UI 전면 리디자인** (2026-07-27~28 작업, 아직 미커밋·미머지) — webdesigner 에이전트로 화면별 HTML 시안을 뽑아 실제 컴포넌트에 반영하는 중. 변경 규모 20파일 +885/-344.
  - 시안 생성 스크립트(레포 루트): `call-webdesigner.mjs`, `generate-webdesigner-code.mjs`, `generate-webdesigner-image.mjs`
  - 시안 산출물(레포 루트): `webdesigner-admin-{dashboard,workers,sites,worklogs,settlements,accounts}-1.html/png`, `webdesigner-history-redesign-1`, `webdesigner-settlement-redesign-1`, `webdesigner-clockscreen-3options.md`
  - 신규 파일: `src/components/BottomNav.jsx`(용역자 하단 탭바 — 홈/내 작업내역/정산내역/로그아웃), `src/components/SiteFilterChips.jsx`, `src/hooks/useWorkLog.js`, `src/hooks/useMyWorkHistory.js`, `src/lib/devMode.js`
  - 구조 변경: 용역자 화면이 "삼선메뉴 + 팝업" → **하단 탭바** 구조로 전환. `lucide-react` 신규 의존성 추가.
  - 개발용 빠른 로그인(`devMode.js`): `npm run dev` + `?dev=1`일 때만 동작, 운영 빌드에선 강제 비활성. 테스트 계정 `010-9999-9999`(용역자) / `testadmin`(관리자)
  - 마지막 흔적: 2026-07-28 23:29 `src/lib/format.js` 수정 후 `attendance/dist` 빌드 (배포 여부 미확인)
...
- [ ] **월 선택 UI 변경 (논의 대기)** — 내 작업내역/정산내역 등에서 쓰는 브라우저 기본 `<input type="month">`가 브라우저마다 UI가 달라 보임(크롬은 달력그리드). "◀ 2026년 7월 ▶" 화살표 전/후 전환 UI로 교체 검토 중 — 나중에 다시 논의 후 진행. (2026-07-28 추가)
...
- [x] 2026-07-20 — **출퇴근 툴 커밋 + main 머지** — 그동안 미커밋으로 쌓여있던 전체를 커밋(a8cd510). firebase.json 멀티사이트(main/attendance) 전환 여파로 GitHub Actions 자동배포가 실패하는 문제 발견 → 워크플로에 `target: main` 지정으로 수정(c871aed).
