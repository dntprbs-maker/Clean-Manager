...
");
    ...
    for(let i=0; i<lines.length; i++) {
      ...
      if(line === "BEGIN:VEVENT") {
        ...
        if(line.startsWith("SUMMARY:")) {
          ...
          current.start = val.length >= 8 ? val.slice(0,4)+"-"+val.slice(4,6)+"-"+val.slice(6,8) : val;
          if(val.length > 8) {
          ...
          const val = line.split(":").pop().trim();
          current.end = val.length >= 8 ? val.slice(0,4)+"-"+val.slice(4,6)+"-"+val.slice(6,8) : val;
          if(val.length > 8) {
            const h = val.slice(9,11), m = val.slice(11,13);
            current.endTime = h+":"+m;
          }
  ...
  const handleFile = (e) => {
    ...
    if(!file.name.endsWith(".ics")) {
      setError(".ics 파일만 업로드 가능합니다."); return;
    }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white px-8 text-center">
        ...
        <p className="text-sm text-gray-500 mb-8">
          {selectedIds.length}개 일정이 캘린더에 추가됐어요.
        </p>
    ...
    return (
      <div className="flex-1 flex flex-col bg-gray-50 min-h-screen">
        ...
        <div className="px-4 py-3 flex items-center gap-3 bg-white border-b border-gray-100">
          <button onClick={()=>setSelectedIds(selectedIds.length===parsedEvents.length?[]:[...parsedEvents.keys()])}
        ...
        {/* 하단 버튼 */}
        <div className="fixed bottom-0 left-0 right-0 px-4 pb-8 pt-3 bg-white border-t border-gray-100" style={{maxWidth:430,margin:"0 auto"}}>
